...
def LargeSoluction(shots)-> dict:
    ...
    dic = {key:val for key, val in counts.items() if val > shots/len(counts.items())} #Filter
    dic[-1] = 0
    bad_sol = 0
    ...
    dic[-1] = dic[-1]/bad_sol #Computing measures of not correct solutions
    dic = {key:val/shots for key, val in dic.items()} # % counts
